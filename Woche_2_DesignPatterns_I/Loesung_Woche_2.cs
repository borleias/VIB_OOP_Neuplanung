...
public class KonfigurationsManager
{
    ...
    {
        string wert;
        if (!_configs.TryGetValue(schluessel, out wert))
        {
            throw new ArgumentException("Key not found.");
        }
        return wert;
    }
    public void SetzeEinstellung(string schluessel, string wert)
    {
        if (!_configs.TryAdd(schluessel, wert))
        {
